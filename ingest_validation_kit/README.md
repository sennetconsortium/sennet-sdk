...
SenNet data upload guidelines, and tools which check that uploads adhere to those guidelines.
Assay documentation is on [Github Pages](https://hubmapconsortium.github.io/ingest-validation-tools/).

SenNet has three distinct metadata processes:
- **Source** 
...
git clone https://github.com/sennetconsortium/ingest-validation-tools.git
cd ingest-validation-tools
# Optionally, set up venv or conda, then:
python -m venv sennet-ingest-vtools
source sennet-ingest-vtools/bin/activate
...
Documentation and metadata TSV templates for each assay type are [here](https://docs.sennetconsortium.org/libraries/ingest-validation-tools/).
...
Data upload to SenNet is composed of discrete phases:
